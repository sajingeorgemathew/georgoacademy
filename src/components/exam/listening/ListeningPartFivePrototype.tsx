...
import { listeningPartFiveReviewCopy } from "@/features/exam-engine/listening-review-copy";
...
//
// What it does share with Parts 2, 3 and 4 is how the part closes. All
// four keep their answer key on the server and ask for it to be marked,
// so the marking state machine below is the Part 4 one, and the three
// closing screens are the same components with Part 5 wording passed in.
//
...
//
// Part 5 has one video and eight questions, so the flow is seven screens:
// part intro, scenario, discussion video, the question screen, the answer
// review, the practice score, and the end of part screen. It was five in
// EXAM-11, which closed on a single completion screen. The only change
// behind that was dropping the EXAM-11 ending from the
// buildListeningVideoFlow call, which now defaults to "review".
//
...
// on the question screen with all eight options still selected, which is
// the prototype affordance the ticket asks for.
//
...
export type ListeningPartFiveMarkAction = (
...
export type ListeningPartFivePrototypeProps = {
  ...
  markAnswers: ListeningPartFiveMarkAction;
};
...
export function ListeningPartFivePrototype({
  ...
  // The default "review" ending now, rather than the EXAM-11 single
  ...
  const requestMarking = async (submitted: ListeningVideoAnswerMap) => {
  ...
  //
  // The part-complete kind belongs to the EXAM-11 ending, which this flow

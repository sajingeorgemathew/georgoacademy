...
// Practice score for Mock Test 1 Reading Part 3 (EXAM-21).
//
// The screen a learner reaches when they finish the part. It shows the
// summary the server action counted, correct out of the nine statements,
// and offers the three ways out: the answer review, a restart, and the
// dashboard.
//
// Everything on it that carries a number comes from
// ReadingScoreSummaryCard, the card Reading Parts 1 and 2 use, unchanged.
// The card is already part neutral: it labels the counts, prints the
// practice score, and carries both the blank note and the "not an
// official CELPIP score" note, so there is nothing about the earlier
// parts in it to strip out. This file is the frame around it.
//
// It is a separate file from ReadingPartTwoScoreScreen rather than a
...
// label, and the Part 1 and Part 2 screens are working and shipped. A
// shared screen would mean editing two live routes' components to add a
// third part, which the ticket asks us not to do without reason. Every
// reusable piece here already is reused: the shell, the instruction row,
// the button, and the card doing all the counting.
//
...
export type ReadingPartThreeScoreScreenProps = {
  title: string;
...
export function ReadingPartThreeScoreScreen({
  title,
  summary,
  onReviewAnswers,
  onRestart,
  ...
  onBack,
  showBack = true,
}: ReadingPartThreeScoreScreenProps) {
  return (
    <ExamShell
      title={title}
      metaText={metaText}
    ...
    >
      <div className={examScreenBody.stack}>
        <ExamInstructionRow
          heading={readingReviewCopy.partThreeScoreTitle}
          ...
          <div className={examScreenBody.actions}>
            ...
            {onRestart ? (
              <ExamButton
                ...
                uppercase={false}
              >
                {readingCopy.partThreeRestartLabel}
              </ExamButton>
            ) : null}
            <ExamButton
              variant="secondary"
